...
- Caps live under `ai.*`: `max_issues_per_run` (default 100), `max_input_tokens`, `max_daily_cost_usd`, `max_tokens_per_issue`, `samples_per_issue`, `max_source_file_bytes`. Every cap is optional — `null` disables it. For a pure money budget set `max_issues_per_run` and `max_input_tokens` to `null` and `max_daily_cost_usd` to e.g. `1.00`; it sums actually billed cost per calendar day across all runs.
